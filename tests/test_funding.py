...
def test_gauge_state_is_captured_before_the_deposit_snapshot_is_restored(monkeypatch):
    fork = FundingFork()
    simulator = object.__new__(Simulator)
    simulator.prices = UnitPrice()
    router = Router(fork, simulator.prices)
    strategy = next(s for s in strategies() if s.name == "curve-3pool-gauge")
    verified = VerifiedStrategy(strategy, fork.block, strategy.gauge, None, "code")
    expected = GaugeState(123, 456, 456, fork.block.timestamp, ())
    monkeypatch.setattr(simulator, "costs", lambda block, gas: (gas, Decimal(gas)))
    ...
    def capture(fork, verified):
        assert fork.transactions == [{"amount": 90 * 10**6, "gas_units": 10}]
        return expected

    monkeypatch.setattr("hodl.simulation.deposit", deposit)
    monkeypatch.setattr("hodl.simulation.gauge_state", capture)
    result = simulator.funded_deposit(fork, router, verified, USDC, 100 * 10**6)
    assert result.amount == 90 * 10**6
    assert result.gauge_state == expected
    assert result.gas_usd == 10
    assert fork.transactions == []
    assert fork.gas_units == 0
